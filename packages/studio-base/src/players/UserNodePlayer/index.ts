  ...
  private async _createNodeRegistration(
    ...
    const buildMessageProcessor = (): NodeRegistration["processMessage"] => {
      return async (msgEvent: MessageEvent<unknown>, globalVariables: GlobalVariables) => {
        ...
        if (!rpc) {
          ...
          if (!rpc) {
            ...
            rpc.receive("error", (msg) => {
              ...
              this._problemStore.set(problemKey, {
                severity: "error",
                message: `User scripts runtime error: ${msg}`,
              });
        ...
        if (!result) {
          this._problemStore.set(problemKey, {
            message: `User scripts node ${nodeId} timed out`,
            severity: "warn",
          });
          return;
        }
        ...
        if (!result.message) {
          this._problemStore.set(problemKey, {
            severity: "warn",
            message: `User scripts node ${nodeId} did not produce a message`,
          });
          return;
        }
    ...
    if (!this._nodeTransformRpc) {
      ...
      worker.onerror = (event) => {
        ...
        this._problemStore.set("worker-error", {
          severity: "error",
          message: `User scripts error: ${event.message}`,
        });
      ...
      rpc.receive("error", (msg) => {
        ...
        this._problemStore.set("worker-error", {
          severity: "error",
          message: `User scripts error: ${msg}`,
        });
